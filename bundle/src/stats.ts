...
export function initStats() {
	...
	if (process.getuid && process.getuid() === 0) {
		console.warn(
			red(
				`[Process] Warning copycord is running as root, this highly discouraged and might expose your system vulnerable to attackers. Please run copycord as a user without root privileges.`
			)
		);
	}
