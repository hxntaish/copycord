...
export class MessageFlags extends BitField {
	static FLAGS = {
		...
		// SOURCE_MESSAGE_DELETED: BigInt(1) << BigInt(3), // copycord will delete them from destination too, making this redundant
		URGENT: BigInt(1) << BigInt(4),
		// HAS_THREAD: BigInt(1) << BigInt(5) // does not apply to copycord due to infrastructural differences
