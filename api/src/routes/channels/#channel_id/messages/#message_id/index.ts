...
import { handleMessage, postHandleMessage } from "@copycord/api";
...
router.put(
	...
	async (req: Request, res: Response) => {
		...
		if (Date.now() < snowflake.timestamp) {
			...
			throw copycordApiErrors.CANNOT_BACKFILL_TO_THE_FUTURE;
		}

		const exists = await Message.findOne({ where: { id: message_id, channel_id: channel_id }});
		if (exists) {
			throw copycordApiErrors.CANNOT_REPLACE_BY_BACKFILL;
		}
